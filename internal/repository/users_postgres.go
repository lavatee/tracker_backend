...
func (r *UsersPostgres) SignUp(user model.User) (int, error) {
	var id int
	query := fmt.Sprintf("INSERT INTO %s (telegram_username, first_name, last_name, grade, class_letter, password_hash, referral, by_referral) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id", usersTable)
	row := r.db.QueryRow(query, user.TelegramUsername, user.FirstName, user.LastName, user.Grage, user.ClassLetter, user.PasswordHash, user.Referral, user.ByReferral)
...
func (r *UsersPostgres) SignIn(telegramUsername string, passwordHash string) (model.User, error) {
	var user model.User
	query := fmt.Sprintf("SELECT id FROM %s WHERE telegram_username = $1 and password_hash = $2", usersTable)
...
func (r *UsersPostgres) GetUserReferrals(userId int) ([]model.User, error) {
	...
	query := fmt.Sprintf("SELECT first_name, last_name, telegram_username, grade, class_letter FROM %s WHERE by_referral", usersTable)
